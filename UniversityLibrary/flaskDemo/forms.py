...
from flaskDemo.models import User, Department, getDepartment, getDepartmentFactory, Works_on, Employee, Project
...
#MY CODE BELOW
projs = Project.query.with_entities(Project.pnumber).distinct()
projectChoices2=[(row[0],row[0]) for row in projs]
results2=list()
for row in projs:
    rowDict=row._asdict()
    results.append(rowDict)
projectChoices = [(row['pnumber'],row['pnumber']) for row in results2]
...
#  One of many ways to use SelectField or QuerySelectField.  Lots of issues using those fields!!
    mgr_ssn = SelectField("Select Manager SSN", choices=myChoices)  # myChoices defined at top
...
class AssignmentForm(FlaskForm):
    ...
    pno = SelectField("Select Project", choices=projectChoices2)
#  Commented out using a text field, validated with a Regexp.  That also works, but a hassle to enter ssn.
#    mgr_ssn = StringField("Manager's SSN", validators=[DataRequired(),Regexp('^(?!000|666)[0-8][0-9]{2}(?!00)[0-9]{2}(?!0000)[0-9]{4}$', message="Please enter 9 digits for a social security.")])

#  One of many ways to use SelectField or QuerySelectField.  Lots of issues using those fields!!
    essn = SelectField("Select Employee By SSN", choices=myChoices)  # myChoices defined at top
    submit = SubmitField('Make this Assignment')
